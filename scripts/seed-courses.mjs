 ...
 * Mystic Pines is a 9-hole course (par 36, Silver 34.8/127).
 * Verchères: main Verchères course (not Madeleine). This script also merges
 * the legacy score name "Vercheres" into "Club De Golf Vercheres" (idempotent).
 */
...
const COURSES = [
  ...
  { name: 'Mystic Pines',                   tee_name: 'Silver', course_rating: 34.8, slope_rating: 127, par: 36, holes: 9 },
]
...
async function main() {
  ...
  // Consolidate legacy score name "Vercheres" → "Club De Golf Vercheres".
  // Idempotent: after the first run there are no "Vercheres" rounds left.
  const merged = await prisma.score.updateMany({
    where: { course_name: 'Vercheres' },
    data:  { course_name: 'Club De Golf Vercheres' },
  })
  if (merged.count > 0) {
    console.log(`↻ Merged ${merged.count} 'Vercheres' round(s) into 'Club De Golf Vercheres'`)
  }
